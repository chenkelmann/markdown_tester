...
class Metronome(val frequency:Int, val offset:Int) {
    var counter = offset
    def tick:Boolean = {
        counter += 1
        if (counter % frequency == 0) true //tick
        else                          false //tock
    }
}

abstract trait MarkdownTestGenerator {
    def word():String
    def line(charLimit:Int):String
    def blankLines(count:Int):String
    def block(lines:Int, charLimitPerLine:Int):String
}

class TestText(val template:String, val newLine:String = "\n") extends MarkdownTestGenerator{
    //repeat template text indefinitely
    var stream:Stream[String] = {
        val tokenizer = new StringTokenizer(template, " ", false)
        val words = for (i <- 0 until tokenizer.countTokens) yield tokenizer.nextToken
        Stream.continually(words.toStream).flatten
    }
    ...
    def block(lines:Int, charLimitPerLine:Int):String = {
        {for(i <- 0 until lines) yield line(charLimitPerLine)}.mkString(newLine)
    }

    def document(blocks:Int, linesPerBlock:Int, charLimitPerLine:Int):String = {
        {for (i <- 0 until blocks) yield block(linesPerBlock, charLimitPerLine) + blankLines(2)}.mkString("")
    }
}

class LinePrepender(blockFrequency:Int, blockOffset:Int, prefix:String) {
    var active = false
    val metronome = new Metronome(blockFrequency, blockOffset)

    def line(l:String) = if (active) prefix + l
                         else l

    def blockStart() = active = metronome.tick

}

abstract trait Blockquote extends MarkdownTestGenerator{
    def blockquoteFrequency = 6
    def blockquoteOffset = 0

    val blockquotePrepender = new LinePrepender(blockquoteFrequency, blockquoteOffset, "> ")

    abstract override def line(charLimit:Int):String = blockquotePrepender.line(super.line(charLimit))

    abstract override def block(lines:Int, charLimitPerLine:Int):String = {
        blockquotePrepender.blockStart()
        super.block(lines, charLimitPerLine)
    }
}

abstract trait Codeblock extends MarkdownTestGenerator{
    def codeblockFrequency = 6
    def codeblockOffset    = 1

    val codeblockPrepender = new LinePrepender(codeblockFrequency, codeblockOffset, "    ")

    abstract override def line(charLimit:Int):String = codeblockPrepender.line(super.line(charLimit))

    abstract override def block(lines:Int, charLimitPerLine:Int):String = {
        codeblockPrepender.blockStart()
        super.block(lines, charLimitPerLine)
    }
}

abstract trait UList extends MarkdownTestGenerator{
    def ulistFrequency = 6
    def ulistOffset    = 2

    val ulistPrepender = new LinePrepender(ulistFrequency, ulistOffset, "* ")

    abstract override def line(charLimit:Int):String = ulistPrepender.line(super.line(charLimit))

    abstract override def block(lines:Int, charLimitPerLine:Int):String = {
        ulistPrepender.blockStart()
        super.block(lines, charLimitPerLine)
    }
}

class WordWrapper(val wraps:List[(String, String)], val frequency:Int, val offset:Int) {
    var stream:Stream[(String, String)] = Stream.continually(wraps.toStream).flatten
    val metronome = new Metronome(frequency, offset)
    def wrap(word:String) = {
        if (metronome.tick) {
            val (begin, end) = stream.head
            stream = stream.tail
            begin + word + end
        } else {
            word
...
abstract trait Emphasis extends MarkdownTestGenerator {
    def emphasisFrequency = 15
    def emphasisOffset = 0

    val emphasisWrapper = new WordWrapper(List(("*", "*"), ("_", "_")), emphasisFrequency, emphasisOffset)

    abstract override def word() = emphasisWrapper.wrap(super.word)
}

abstract trait Strong extends MarkdownTestGenerator {
    def strongFrequency = 15
    def strongOffset = 1

    val strongWrapper = new WordWrapper(List(("**", "**"), ("__", "__")), strongFrequency, strongOffset)

    abstract override def word() = strongWrapper.wrap(super.word)
}

abstract trait Code extends MarkdownTestGenerator {
    def codeFrequency = 15
    def codeOffset = 2

    val codeWrapper = new WordWrapper(List(("`", "`"), ("``", "``")), codeFrequency, codeOffset)

    abstract override def word() = codeWrapper.wrap(super.word)
}


abstract trait XmlSpecial extends MarkdownTestGenerator {
    def xmlSpecialFrequency = 15
    def xmlSpecialOffset = 3

    val xmlSpecialMetronome = new Metronome(xmlSpecialFrequency, xmlSpecialOffset)

    abstract override def word() = if (xmlSpecialMetronome.tick) "\"><'&"
                                   else (super.word)
}

abstract trait ManualBreak extends MarkdownTestGenerator {
    def breakFrequency = 10
    def breakOffset    = 0
    val breakMetronome = new Metronome(breakFrequency, breakOffset)
    abstract override def line(charLimit:Int) = if (breakMetronome.tick) super.line(charLimit:Int) + "  \n"
                                                else                super.line(charLimit:Int)
}

abstract trait FastLink extends MarkdownTestGenerator {
    def fastLinkFrequency = 15
    def fastLinkOffset = 4

    val fastLinkWrapper = new WordWrapper(List((" <http://www.example.com/", ">")), fastLinkFrequency, fastLinkOffset)

    abstract override def word() = fastLinkWrapper.wrap(super.word)
}

abstract trait FullLink extends MarkdownTestGenerator {
    def fullLinkFrequency = 15
    def fullLinkOffset = 5
    val fullLinkMetronome = new Metronome(fullLinkFrequency, fullLinkOffset)

    abstract override def word () = {
        val w = super.word()
        if (fullLinkMetronome.tick) {
...
abstract trait FullImage extends MarkdownTestGenerator {
    def fullImageFrequency = 15
    def fullImageOffset = 6
    val fullImageMetronome = new Metronome(fullImageFrequency, fullImageOffset)

    abstract override def word () = {
        val w = super.word()
        if (fullImageMetronome.tick) {
            "![" + w + "](/path/to/" + w + ".jpg)"
        } else {
            w
        }
...
abstract trait RefLink extends MarkdownTestGenerator {
    def refLinkFrequency = 16
    def refLinkOffset = 7
    val refLinkMetronome = new Metronome(refLinkFrequency, refLinkOffset)
    var id = 0
    val linkDefs = new ArrayBuffer[(String, String)]()

    abstract override def word () = {
        val w = super.word()
        if (refLinkMetronome.tick) {
            id += 1
            val idString = "id" + id
            linkDefs.append((idString, w))
            "[" + w + "][" + idString + "]"
        } else {
            w
        }
    ...
    abstract override def block(lines:Int, charLimitPerLine:Int) = {
        val b = super.block(lines:Int, charLimitPerLine:Int)
        def pairToDefLine(pair:(String, String)):String = {
            val (idString, w) = pair
            "[" + idString + "]: http://example.com/" + w + " \"Title:" + w + "\" \n"
        }
        val defLines = linkDefs.map(pairToDefLine(_)).mkString("")
        b + "\n\n" + defLines + "\n\n"
    }
}

abstract trait InlineHtml extends MarkdownTestGenerator {
    def inlineHtmlFrequency = 15
    def inlineHtmlOffset = 8
    def inlineHtmlTags:List[(String, String)] = List((" <blink>", "</blink>"), (" <span class=\"markdown\">", "</span>"))

    val inlineHtmlWrapper = new WordWrapper(inlineHtmlTags, inlineHtmlFrequency, inlineHtmlOffset)

    abstract override def word() = inlineHtmlWrapper.wrap(super.word)
}

object TextGenerator {
   /*
    def main(args:Array[String]) {
        val generator = new TestText(loremIpsum) with Emphasis with XmlSpecial with Strong with Blockquote{
            override def strongOffset = 0
        }
        println(generator.document(10, 10, 40))
    }
     */
    val loremIpsum = """"Non eram nescius, Brute, cum, quae summis ingeniis exquisitaque doctrina philosophi Graeco sermone tractavissent, ea Latinis litteris mandaremus, fore ut hic noster labor in varias reprehensiones incurreret. nam quibusdam, et iis quidem non admodum indoctis, totum hoc displicet philosophari. quidam autem non tam id reprehendunt, si remissius agatur, sed tantum studium tamque multam operam ponendam in eo non arbitrantur. erunt etiam, et ii quidem eruditi Graecis litteris, contemnentes Latinas, qui se dicant in Graecis legendis operam malle consumere. postremo aliquos futuros suspicor, qui me ad alias litteras vocent, genus hoc scribendi, etsi sit elegans, personae tamen et dignitatis esse negent.
